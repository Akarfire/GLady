...
* `Main.py` - contains plugin's class;
* `plugin_info.txt` - plugin's meta data;
* `Config` folder - contains plugin's configuration files:
	* `Options.txt` - contains general plugin options;
	* `EventMapping.txt` - specifies mapping of incoming (received) events to corresponding Event Processor Functions (EPFs);
	* `EventGeneration.txt` - applies configurable names to event's generated by the plugin.
...
inheritance = SomeName, ...
dependencies = SampleDependencyPlugin, ...
```

* *name* - name of this plugin;
* *version* - version of this plugin;
* *enabled* - whether the plugin is enabled or not (disabled plugins are not loaded)
* *description* - description of this plugin;
* *code_file* - relative path to the main .py file;
* *inheritance* - list of names used for dependency resolution. For example if plugin A required dependency B, any plugin that contains B in it's inheritance list will suffice the requirement. 
